...
@router.get(
    ...
    response_model=Mappings,
    summary="Returns mappings for a specific mapping type and source system.",
    description=dedent(
        """
        Retrieves mappings for the specified mapping_type and source_system
        from the project's .fmu directory.

        Example: GET /project/mappings/stratigraphy/rms returns the
        stratigraphy mappings whose source system is RMS.
        """
    ),
    responses={
        **GetSessionResponses,
...
async def get_mappings(
    ...
    """Returns mappings for a specific mapping type and source system."""
    try:
        return mappings_service.get_mappings_by_source_system(
            mapping_type,
            source_system,
        )
    except FileNotFoundError as e:
...
@router.put(
    ...
    response_model=Message,
    dependencies=[WritePermissionDep, RefreshLockDep],
    summary="Updates mappings for a specific mapping type and source system",
    description=dedent(
        """
        Replaces mappings for the specified mapping_type and source system in
        the project's .fmu directory.

        The request body should contain the mappings collection for the
...
async def put_mappings(
    ...
    """Updates mappings for a specific mapping type and source system."""
    try:
